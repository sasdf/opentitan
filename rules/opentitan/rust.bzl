...
def _rust_library(name, rustc_flags = [], crate_name = None, mod_name = None, **kwargs):
    crate_name = _compute_crate_name(name, crate_name = crate_name, mod_name = mod_name)

    platform_flags = select({
        "//rules:opentitan_platform": [
            "--cfg=ot_platform_rv32",
            "--cfg=panic_immediate_abort",
            "-Cpanic=abort",
        ],
        "//conditions:default": [],
    })
    rustc_flags = RUSTC_FLAGS + platform_flags + rustc_flags
...
def dual_cc_library_with_bindgen(
    ...
    if "mod_name" not in bindgen:
        bindgen["mod_name"] = name
...
def _rust_bindgen_diff(name, base, bindings, crate_import, testonly = False):
    native.genrule(
        ...
        cmd = """
          ...
          > $@
      """.format(
            rust_bindgen_diff = "$(locations //util/design:rust_bindgen_diff) ",
        ...
        tools = ["//util/design:rust_bindgen_diff"],
        toolchains = [
...
def _rust_bindgen_filter(name, bindings, testonly = False, **kwargs):
    ...
    native.genrule(
        ...
        cmd = """
          ...
          > $@
      """.format(
            rust_bindgen_filter = "$(locations //util/design:rust_bindgen_filter) ",
            rustfmt = "$(locations @rules_rust//rust/toolchain:current_rustfmt_files) ",
            bindings = "$(locations " + bindings + ") ",
            blocklist = "$(locations " + blocklist + ") ",
        ),
        tools = ["//util/design:rust_bindgen_filter"],
        toolchains = [
            "@rules_rust//rust/toolchain:current_rustfmt_files",
        ],
    )
...
def _get_crate_import_impl(ctx):
    ...
    for dep in ctx.attr.exports:
        ...
        imports.append('#[doc = "' + str(dep[CrateInfo].owner) + '"]\n')
