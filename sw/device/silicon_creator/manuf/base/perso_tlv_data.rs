...
macro_rules! perso_tlv_get_field {
    ($type_name:ident, $field_name: ident, $full_value: ident) => {{
        let mask: u16 = concat_idents!(k, $type_name, $field_name, FieldMask) as u16;
        let shift: u16 = concat_idents!(k, $type_name, $field_name, FieldShift) as u16;
...
pub fn perso_tlv_get_cert_obj_rust(buf: &[u8], obj: &mut perso_tlv_cert_obj_t) -> RomError {
    ...
    if buf.len() < size_of::<perso_tlv_object_header_t>() {
        return RomError(kErrorPersoTlvCertObjNotFound); // Not enough data for header
    }
