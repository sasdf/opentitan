...
impl UartConsole {
        ...
        T: ConsoleDevice + ?Sized,
    {
        ...
        if self.exit_success.is_some() || self.exit_failure.is_some() {
            self.exit_plugin = ExitPlugin::default()
                .set_exit_success(std::mem::take(&mut self.exit_success))
                .set_exit_failure(std::mem::take(&mut self.exit_failure));
        }

        if stdout.is_none() {
            self.logging_plugin.quiet = true;
        }
    ...
    fn get_exit_status(&mut self) -> Option<ExitStatus> {
        ...
        if let Some(status) = self.exit_plugin.get_exit_status() {
            return Some(status);
        }
        None
    }
        ...
        T: ConsoleDevice + ?Sized,
    {
        let mut console = UartConsole {
            ...
            exit_plugin: ExitPlugin::default().set_exit_success(Some(Regex::new(rx)?)),
            ..Default::default()
        };
