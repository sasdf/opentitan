...
impl CoveragePlugin {
    pub fn set_stop_after_report(mut self) -> Self {
        self.should_stop_after_report = true;
        self
    }
    ...
    fn save_coverage_data(&mut self) -> Result<()> {
        ...
        eprintln!("INFO: Saving coverage profile to {path}");
        std::fs::write(path, response)?;

        Ok(())
    }

    fn set_exit_status(&mut self, status: ExitStatus) {
        if self.should_stop_after_report {
            eprintln!("INFO: Stop console after receiving coverage report.");
            self.exit_status = Some(status);
        }
    }

    fn process_byte(&mut self, byte: u8) -> Result<Vec<u8>> {
        self.buffer.push(byte);
        if self.buffer.ends_with(COVERAGE_START_ANCHOR) {
            if self.is_capturing {
                eprintln!("WARN: Got unterminated coverage block, starting new one.");
            } else {
                eprintln!("INFO: Coverage capture started.");
            }
            ...
            if self.is_capturing {
                ...
                eprintln!("INFO: Coverage capture ended.");
            } else {
                eprintln!("WARN: Got unexpected coverage end indicator!");
            }
            self.is_capturing = false;
            self.buffer.clear();
            self.set_exit_status(ExitStatus::ExitSuccess);
            Ok(Vec::new())
        } else if self.buffer.ends_with(COVERAGE_SKIP_ANCHOR) {
            if self.is_capturing {
                eprintln!("WARN: Got unterminated coverage block, skipping it.");
            } else {
                eprintln!("INFO: Coverage capture skipped.");
            }
            self.is_capturing = false;
            self.buffer.clear();
            self.set_exit_status(ExitStatus::ExitSuccess);
...
impl ConsolePlugin for CoveragePlugin {
    fn process_bytes(&mut self, bytes: Vec<u8>) -> Result<Vec<u8>> {
        ...
        for byte in bytes {
            result_bytes.extend(self.process_byte(byte)?);
        }
        Ok(result_bytes)
    }
