...
fn load_bindgen() -> syn::File {
    ...
    let file_contents = std::fs::read_to_string(path).expect("Failed to read bindings");
    let ast: syn::File = syn::parse_str(&file_contents).expect("Failed to parse bindings");
    ast
}
...
pub fn include(_: TokenStream) -> TokenStream {
    ...
    // Group all externs together.
    let mut externs = Vec::new();
    let mut others = Vec::new();
    for item in ast.items {
        match item {
            syn::Item::ForeignMod(foreign_mod) => externs.push(foreign_mod),
            _ => others.push(item),
        }
    }
